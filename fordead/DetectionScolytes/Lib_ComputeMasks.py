...
from Lib_WritingResults import writeBinaryRaster

def getMaskForet(PathMaskForet):
...
def ComputeMaskForet(PathExampleBand,InputDirectory,ForestMaskSource,tuile):
    ...
    if ForestMaskSource=="LastComputed":
        RasterizedBDFORET,profile,CRS_Tuile = getMaskForet(os.path.join(InputDirectory,"Rasters",tuile,"MaskForet_"+tuile+".tif"))
        print("Import du raster BDFORET")
    elif ForestMaskSource=="BDFORET":
        RasterizedBDFORET, profile, CRS_Tuile = RasterizeMaskFromBDForet(PathExampleBand,InputDirectory)
        if not(os.path.exists(os.path.join(InputDirectory,"Rasters"))): os.mkdir(os.path.join(InputDirectory,"Rasters"))
        if not(os.path.exists(os.path.join(InputDirectory,"Rasters",tuile))): os.mkdir(os.path.join(InputDirectory,"Rasters",tuile))
        writeBinaryRaster(RasterizedBDFORET,os.path.join(InputDirectory,"Rasters",tuile,"MaskForet_"+tuile+".tif"),profile)
    else:
        print("Source du masque foret inconnu")
        
    return RasterizedBDFORET,profile,CRS_Tuile

def RasterizeMaskFromBDForet(PathExampleBand,InputDirectory):
...
#       TuilePath=os.getcwd()+"/Data/Vecteurs/TilesSentinel.shp"
    with rasterio.open(PathExampleBand) as exampleRaster:
        ...
        return RasterizedBDFORET, exampleRaster.profile, CRS_Tuile
