...
import click
from fordead.import_data import import_coeff_model, import_dieback_data, initialize_dieback_data, import_masked_vi, import_first_detection_date_index, TileInfo, import_forest_mask
from fordead.writing_data import write_tif
from fordead.dieback_detection import detection_anomalies, detection_dieback
...
def dieback_detection(
    ...
    if tile.parameters["Overwrite"] : 
        tile.delete_dirs("AnomaliesDir","state_dieback" ,"confidence_index","periodic_results_dieback","result_files","timelapse","series") #Deleting previous detection results if they exist
    ...
    if  len(new_dates) == 0:
        ...
        for date_index, date in enumerate(tile.dates):
            if date in new_dates:
                ...
                dieback_data = detection_dieback(dieback_data, anomalies, masked_vi["mask"], date_index)
