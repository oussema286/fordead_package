...
```
Here, the model is adjusted based on all Sentinel-2 acquisitions from the first acuisition to last acquisition before 2018-01-01. 
If less than 10 valid acquisitions are available on 2018-01-01, additional acquisition are used in order to reach 10 valid acquisitions, unless this number is not reach on 2018-06-01. 
...
It is not recommended to end the training before 2018, as a minimum of two seasons of of SENTINEL-2 acquisitions is required to adjust the model.
...
- **DataModel** directory :
    - **first_detection_date_index.tif** corresponds to a raster file containing the index of the first acquisition used for detection. 
...
- **ForestMask** directory: the binary raster **valid_area_mask.tif** is written. This mask identify valid pixels for which a model was computed as 1, and non-valid pixels as 0.
